...
`;
    ...
    // Cache for edited eLabFTW preview bodies (keyed by elabid)
    window.elabEditedBodies = {};
...
<script>
  ...
  new vis.Network(container, { nodes, edges }, options);
    ...
    window.runPreviewLLM = async function() {
      // Auto-save any pending edits before running LLM analysis
      saveElabPreviewEdit();
...
`;
    ...
    /**
     * Saves the edited content from the eLabFTW preview offcanvas.
     * Stores it in a global cache so it can be used during conversion.
     */
    function saveElabPreviewEdit() {
      const expContentEl = document.getElementById('expContent');
      if (!expContentEl) return;

      const elabid = window.elabJSON?._previewElabId || window.elabJSON?.id || window.elabJSON?.elabid;
      if (!elabid) {
        showWarningToast('No experiment loaded. Cannot save edits.');
        return;
      }

      const editedHTML = expContentEl.innerHTML;
      window.elabEditedBodies[elabid] = editedHTML;

      // Also update the in-memory JSON so LLM preview uses it
      if (window.elabJSON) {
        window.elabJSON.body = editedHTML;
      }

      showToast('Preview edits saved. They will be used in the next conversion.', 'success', 3000);
    }
    ...
    window.loadExperiment = async function(instance, elabid, elabtoken, type) {
        try {
            ...
            window.elabJSON._previewElabId = elabid;
